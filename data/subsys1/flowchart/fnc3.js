...
function placelabels(){
	...
	labelproc($("#Z1P4Details"),$("#basepic"),constraints.c2,constraints.r1,"center","center",1);
	...
	labelproc($("#Z1P1Details"),$("#basepic"),constraints.c1,constraints.r3,"center","center",1);
	...
	labelproc($("#zone1Info"),$("#basepic"),530,215,"center","center",1);
	labelproc($("#zone2Info"),$("#basepic"),870,215,"center","center",1);
	// labelproc($("#zone3Info"),$("#basepic"),1215,385,"center","center",1);
	labelproc($("#zone3Info"),$("#basepic"),1113,215,"center","center",1);
	...
	sensorPointsCoord.map((point, index) => {
		labelproc($("#sensorPoint"+(index+1)),$("#basepic"),point.x,point.y,"center","center",1);
	});
...
function checkAlarmState(key){
	...
	if (alarmType == 'UNIT' || alarmType == 'MAINS' || alarmType.includes('SB')){
		...
		if ( VarValues[key] ) {
			...
			if (!$('[data-alarm-variable-name="'+key+'"]').length ) {
				$('#zone'+alarmZone+'Info .alarmsList').append('<div data-alarm-variable-name="'+key+'">'+VarGroup[key].description[curr_lang].replace('(#'+alarmZone+') ','').replace('(№'+alarmZone+') ','')+'</div>')
			}
		} else {
			$('[data-alarm-variable-name="'+key+'"]').remove();
		}
...
function generatePointData(pointId, zone, pointTypeNumber){
	...
	generatedFrame += "<div id=\"Z"+zone+"P"+pointId+"DataFrame\" class=\"AddData draggableFrame\" style=\"width:550px;left:"+zone*50+"px;top:"+ ((pointId*30)+(zone < 4 ? 30 : 500)) +"px;display:none;\">";
...
function FlowChartCont(){
	...
	function generateSensorsHtml(sensorsArray) {
		...
		sensorsArray.map((point, index) => {
			...
			if (index == 3 || index == 6 || index == 7 || index == 12 || index == 16 || index == 17){
				...
				varNameTmp = "Z"+zoneNum+(zoneNum === 1 || zoneNum === 3 ? "BACURVAL" : "B6CURVAL");
				// varNameHmd = "Z"+zoneNum+(zoneNum === 1 || zoneNum === 3 ? "BACURVAL" : "B6CURVAL");
				elem = "<div class=\"sensorPoint\" id=\"sensor"+(index+1)+"\" onClick=\"extendSensors("+(index+1)+")\">"
					+"<div class='sensorTmp'>"
						...
						+"<span data-sensor-variable-name=\""+varNameTmp+"\">-</span>"+(VarGroup[varNameTmp] ? VarGroup[varNameTmp].dataunits : '')
					+"</div>"
					// +"<div class='sensorHmd'>"
					// 	+"<span class=\"extendedTitle\">"+titleLabels[curr_lang].humidity+":</span>"
					// 	+"<span class=\"condensedTitle\">φ:</span>"
					// 	+"<span data-sensor-variable-name=\""+varNameHmd+"\">-</span>"+(VarGroup[varNameHmd] ? VarGroup[varNameHmd].dataunits : '')
					// +"</div>"
				+"</div>";
			} else {
				varNameTmp = "SP"+(('0' + (index+1)).slice(-2))+"AIRTMP";
				varNameHmd = "SP"+(('0' + (index+1)).slice(-2))+"AIRHMD";
				elem = "<div class=\"sensorPoint\" id=\"sensor"+(index+1)+"\" onClick=\"extendSensors("+(index+1)+")\">"
					+"<div class='sensorTmp'>"
						+"<span class=\"extendedTitle\">"+titleLabels[curr_lang].temperature+":</span>"
						+"<span class=\"condensedTitle\">t:</span>"
	...
	function generateSensorsPoints(sensorsArray) {
		var result = '';

		sensorsArray.map((point, index) => {
			elem = "<div id=\"sensorPoint"+(index+1)+"\" style=\"width:10px; height:10px; border-radius:50%; background:green; position:absolute;\"></div>";
	...
	$("#basepic").on('load', function(){
		...
		// Sensor points
			...
			+generateSensorsPoints(sensorPointsCoord)
	...
	setInterval(function(){
		...
		if(SrvStat["mb"]["scanning"] == "true"){
			...
			sensorsCoord.map((point, index) => {
				...
				$('[data-sensor-variable-name="'+varNameTmp+'"]').html(VarValues[varNameTmp]);
				$('[data-sensor-variable-name="'+varNameHmd+'"]').html(VarValues[varNameHmd]);
			});
