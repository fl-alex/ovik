...
function ConfTblContent(){
	...
	var labels={
			"uk":{
				"expand_collapse":"Розгорнути / Згорнути все",
			...
			"ru":{
				"expand_collapse":"Развернуть / Свернуть все",
	...
	var html='<br /><div id=\"cnftblcont\"><div class=\"cftb_add\"><img class=\"prcftbl\" src=\"Print.ico\" title=\"'+labels[curr_lang]['print_table']+'\" style=\"cursor:pointer;\" /></div><table class=\"conftable\"><tr><td style=\"cursor:pointer;text-align:right;font-size:10px;color:#808080;\" onClick=\"$(\'.hidden_row\').toggle(100)\" colspan=\"3\">[ '+labels[curr_lang]['expand_collapse']+' ]</td></tr>';
	if(GrNames.length>0){
...
var html='<br /><table class=\"datatable\"><tr><td style=\"cursor:pointer;text-align:right;font-size:10px;color:#808080;\" onClick=\"$(\'.hidden_row\').toggle(100)\" colspan=\"4\">[ '+labels[curr_lang]['expand_collapse']+' ]</td></tr>';
...
var repvarlist=	"<table style=\"border-spacing:0px;width:90%;\">"+		 
				...
				"<tr><td colspan=\"3\" style=\"cursor:pointer;text-align:right;font-size:10px;color:#808080;\" onClick=\"$('\.hidden_row\').toggle(100)\">[ "+labels[curr_lang]['expand_collapse']+" ]</td></tr>"+
			...
			if((VarGroup.hasOwnProperty(Vname))&&
			   ...
			   (
				...
				($("#VarRegFilter").find(".varflt[name='float']").prop("checked")&&(VarGroup[Vname]["type"]=="FLOAT"))
					...
					(JSON.stringify(VarGroup[Vname]["description"]).indexOf($("#varsearchreq_des").val())>-1)
			       )
                            &&(
...
function SrvData(){
	...
	for(var i=0;i<=NodesNames.length-1;i++){
		...
		html=html+"<table style=\"padding:2px;font-size:12px;width:800px;\">"+
			 ...
			 "<tr style=\"background-color:#EAE5DB;\"><td style=\"text-align:left;text-indent:10px;width:300px;\">"+labels[curr_lang]['scanning']+"</td><td>"+(SrvStat[NodesNames[i]]["scanning"] == "true"?labels[curr_lang]['scanning_started']:labels[curr_lang]['scanning_stopped'])+"</td></tr>"+
...
$(document).on("click",".vrlitem",function(){
	...
	if(VarGroup[Name]["origin"]!="virtual"&&!CommObj[VarNodes[Name]][VarGroup[Name]["origin"]]["Port"].includes("COM")){
		Obj.find(".Rtu485").hide();
	}
	if(VarGroup[Name]["origin"]!="virtual"&&!CommObj[VarNodes[Name]][VarGroup[Name]["origin"]]["Port"].includes("ETHERNET")){
...
$(document).on("click",".editpar",function(){
	...
	if(Par!="value"){
		...
		if((Par=="plot")||(Par=="critical")||(Par=="ignorecritical")||(Par=="archive")||(Par=="processing")||(Par=="negval")||(Par=="log")){
			...
			//alert(JSON.stringify(VarGroup[Name][Par]).replaceAll('"','\\"'));
			InptElem="<input type=\"text\" style=\"width:150px;\" id=\"NewParValue\" value=\'"+JSON.stringify(VarGroup[Name][Par])+"\'/>";
		}
		else if((Par=="function")||(Par=="dataunits")){
			InptElem="<input type=\"text\" style=\"width:150px;\" id=\"NewParValue\" value=\'"+VarGroup[Name][Par]+"\'/>";
		}
		else if(Par=="roundto"){
			InptElem="<select id=\"NewParValue\" style=\"width:80px;\">";
